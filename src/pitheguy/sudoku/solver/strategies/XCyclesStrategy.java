...
public class XCyclesStrategy extends SolveStrategy {
    ...
    private List<Square> findStrongLinks(Square square, int digit) {
        ...
        possibleLinks.add(SolverUtils.getOnlySquareThat(square.getSurroundingRow(), s -> s != square && s.getCandidates().contains(digit), true));
        possibleLinks.add(SolverUtils.getOnlySquareThat(square.getSurroundingColumn(), s -> s != square && s.getCandidates().contains(digit), true));
        possibleLinks.add(SolverUtils.getOnlySquareThat(square.getSurroundingBox(), s -> s != square && s.getCandidates().contains(digit), true));
        return possibleLinks.stream().filter(Optional::isPresent).map(Optional::get).toList();
    }
    ...
    private List<Square> findWeakLinksForGroup(List<Square> group, int digit) {
        ...
        List<Square> list = group.stream().filter(s -> !s.isSolved() && s.getCandidates().contains(digit)).collect(Collectors.toList());
